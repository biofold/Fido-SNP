...
def get_input_data(namefile,ucsc_exe,ucsc_dbs,win,dbfasta,dbpps,pklcod,vcf=False):
	...
	for i in input_list:
		if len(i[2])==0:
			print >> sys.stderr, 'WARNING: Incorrect variant -','\t'.join(i[0])
			continue
		elif len(i[4])==0:
			print >> sys.stderr, 'WARNING: Conservation score not available -','\t'.join(i[0])
			continue
		elif len(i[4])==0:
			i[3]=numpy.zeros(2*win+1).tolist()
		else:
			pass
...
if __name__  == '__main__':
	...
	if refgen=='hg38':
		...
		dbpps=['canfam2.phyloP4way.bw','canfam2.phyloP10way.bw']
	elif refgen=='canfam3':
                dbpps=['canfam3.phyloP4way.bw','canfam3.phyloP10way.bw']
	else:
		dbpps=[]
	modfiles=[ prog_dat+ '/snv_model_w5_p7_500_'+refgen+'.pkl',prog_dat + '/indel_model_w5_p7_500_'+refgen+'.pkl']
	#modfiles=[ prog_dat+ '/snv_model_w5_p3_50_'+refgen+'.pkl',prog_dat + '/indel_model_w5_p7_500_'+refgen+'.pkl']
	if dbpps!=[]:
