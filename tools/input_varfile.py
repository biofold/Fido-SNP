...
def global_vars(refgen):
	...
	if refgen=='hg38':                
		...
		dbpps=['canfam2.phyloP10way.bw','canfam2.phyloP10way.bw']
	else:
		print >> sys.stderr,'Incorrect reference genome. Fido-SNP uses only canfam2 and hg38.'
		sys.exit(1)
...
if __name__  == '__main__':
	...
	if dbpps!=[]:
		...
		for data in sdata:
			text='\t'.join([str(i) for i in data[0]])
			seq='\t'.join([str(i) for i in data[2]])
			cons1='\t'.join([str(i) for i in data[3]])
			cons2='\t'.join([str(i) for i in data[4]])
			print '\t'.join([i for i in  [text,seq,cons1,cons2]])
